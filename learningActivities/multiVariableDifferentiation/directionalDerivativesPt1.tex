...
Right now our methods for differentiating rely on having graphs or curves in space. We will first use this to introduce the idea of a derivative at a point on a surface in a certain direction, and then will finish with a more general method for computing these derivatives.
...
Our function is $f(x,y) = xy^2$. We will take a derivative at the point $(0,1)$ in the direction given by $\vec{u} = \langle 1, 2 \rangle$. This selection of a point is the same as picking a point for the derivative in single-variable calculus, the addition of a direction for the derivative is new, and is important for our new context of having multiple input variables.

The direction gives us a line in the domain. The specific line given by $x(t) = \answer{1}t+ \answer{0}$, $y(t) = 2t + \answer{1}$.
...
$$f(x(t), y(t)) = f(t, 2t + 1) = t(2t + 1)^2 = t(4t^2 + 4t + 1) = \answer{4t^3 + 4t^2 + t}$$

The resulting curve in 3D space is $\vec{C}(t) = \langle t, 2t + 1, \answer{4t^3 + 4t^2 + t} \rangle$.

\begin{feedback}
Since $f(x,y) = xy^2$ and we want the derivative at $(0,1)$ in the direction $\vec{u} = \langle 1,2 \rangle$, we can parametrize the line through the domain as:
$$x(t) = 1 \cdot t + 0 = t$$
$$y(t) = 2 \cdot t + 1 = 2t + 1$$

Evaluating the function along this line:
$$f(x(t), y(t)) = f(t, 2t + 1) = t(2t + 1)^2 = t(4t^2 + 4t + 1) = 4t^3 + 4t^2 + t$$
\end{feedback}
\end{problem}
...
\begin{problem}
For the function $f(x,y) = xy^2$ at the point $(0,1)$ in the direction $\vec{u} = \langle 1,2 \rangle$, we found the curve:
$$\vec{C}(t) = \langle t, 2t + 1, 4t^3 + 4t^2 + t \rangle$$
...
Notice that we are explicitly asking about the rate of change only of the height of the hill. This is really important for udnerstanding multivariable derivatives! Since the output of multivariable functions is a single variable, the derivatives tell us only how that output variable changes. All other information is given to us, and relates to the input variables.

Let's answer the question explicitly since we have the curve.

The height of the hill along the curve is given by the third component of $\vec{C}(t)$, which is $f(x(t), y(t)) = 4t^3 + 4t^2 + t$.

Therefore, the rate of change of the hill height as we move along the curve is given by:
$$\frac{d}{dt}[f(x(t), y(t))] = \frac{d}{dt}[4t^3 + 4t^2 + t] = \answer{12t^2 + 8t + 1}$$

At our point of interest $(0,1)$, $t = 0$, so the rate of change of the hill height at $(0,1)$ in the direction $\vec{u}$ is:
$$\frac{d}{dt}[f(x(t), y(t))]\bigg|_{t=0} = \answer{1}$$

This means:

\begin{multipleChoice}
\choice{The hill height does not change as we move in the direction $\vec{u}$ from the point $(0,1)$.}
\choice[correct]{The hill height increases at a rate of 1 unit of height per unit of movement in the direction $\vec{u}$ from the point $(0,1)$.}
\choice{The hill height decreases at a rate of 1 unit of height per unit of movement in the direction $\vec{u}$ from the point $(0,1)$.}
\choice{The hill height increases at a rate of 1 units of height per unit of movement in the $x$ direction from the point $(0,1)$.}
\choice{The hill height increases at a rate of 1 units of height per unit of movement in the $y$ direction from the point $(0,1)$.}
\end{multipleChoice}
\end{problem}
...
\begin{enumerate}
  \item  $f(x,y) = x^2 + y^2$ at the point $(1,2)$ in the direction $\vec{u} = \langle 3,4 \rangle$.
  
  Step 1: Parametrize the line through the domain:
  $$x(t) = \answer{3}t + \answer{1}$$
  $$y(t) = 4t + \answer{2}$$

  Step 2: Create the curve in 3D space:
  $$f(x(t), y(t)) = f(3t + 1, 4t + 2) = (3t + 1)^2 + (4t + 2)^2 = \answer{25t^2 + 22t + 5}$$

  Step 3: Differentiate the height function:
  $$\frac{d}{dt}[f(x(t), y(t))] = \answer{50t + 22}$$

  Step 4: Evaluate at the point of interest. Since we are at $(1,2)$, $t = 0$:
  $$\frac{d}{dt}[f(x(t), y(t))]\bigg|_{t=0} = \answer{22}$$

  \item  $f(x,y) = x^2 + y^2$ at the point $(0,0)$ in the direction $\vec{u} = \langle 1,2 \rangle$.
  
  Step 1: Parametrize the line through the domain:
  $$x(t) = \answer{1}t + \answer{0}$$
  $$y(t) = 2t + \answer{0}$$

  Step 2: Create the curve in 3D space:
  $$f(x(t), y(t)) = f(t, 2t) = t^2 + (2t)^2 = \answer{5t^2}$$

  Step 3: Differentiate the height function:
  $$\frac{d}{dt}[f(x(t), y(t))] = \answer{10t}$$

  Step 4: Evaluate at the point of interest. Since we are at $(0,0)$, $t = 0$:
  $$\frac{d}{dt}[f(x(t), y(t))]\bigg|_{t=0} = \answer{0}$$

  \item  $f(x,y) = cos(x)*sin(y)$ at the point $(\frac{\pi}{2}, \frac{\pi}{2})$ in the direction $\vec{u} = \langle 1,1 \rangle$.
  
  Step 1: Parametrize the line through the domain:
  $$x(t) = \answer{1}t + \answer{\frac{\pi}{2}}$$
  $$y(t) = 1t + \answer{\frac{\pi}{2}}$$

  Step 2: Create the curve in 3D space:
  $$f(x(t), y(t)) = f(t + \frac{\pi}{2}, t + \frac{\pi}{2}) = \cos(t + \frac{\pi}{2}) \cdot \sin(t + \frac{\pi}{2}) = \answer{-\frac{1}{2} \sin(2t)}$$

  Step 3: Differentiate the height function:
  $$\frac{d}{dt}[f(x(t), y(t))] = \answer{-\cos(2t)}$$

  Step 4: Evaluate at the point of interest. Since we are at $(\frac{\pi}{2}, \frac{\pi}{2})$, $t = 0$:
  $$\frac{d}{dt}[f(x(t), y(t))]\bigg|_{t=0} = \answer{-1}$$

  \item Which of the following statements is true about the directional derivatives computed above?
  \begin{multipleChoice}
