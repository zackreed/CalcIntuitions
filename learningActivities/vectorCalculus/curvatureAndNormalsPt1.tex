...
\title{2nd Derivative: Acceleration}
\author{Zack Reed}
...
For a function $f$ where $f''(x)  > 0$, how do the rates of $f$ change as you move from left to right along the graph? (Select All that are true.)
\begin{selectAll}
...
Examining features of curves, such as curvature, requires attention to the measurement process. Let's return to Torty and Harry for a moment. 
...
Now watch the following video to further explore this idea:
...
The following video covers the ways that the unit tangent vector and arc length as a variable can help us make speed independent measurements of curves:
...
$$s=\int^{\answer{1}}_0 |\vec{v}(t)| dt = \int_0^1 \sqrt{4 + 36t^2 + 9t^4} \, dt \approx \answer[tolerance=.01]{3.9337}$$
