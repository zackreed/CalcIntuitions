...
\title{Adding up Pieces: Calculating Volumes}
\author{Zack Reed}
...
Total population: 
$$P = \iint_R dP = \int_{\answer{-2}}^{\answer{2}} \int_{\answer{x^2}}^{\answer{4}} \answer{\frac{100}{1+x^2+y^2}}\cdot\answer{dydx}$$

The order of integration matters because:
\begin{multipleChoice}
    \choice{The density function is complicated}
    \choice[correct]{It might be easier or harder to integrate in a certain order (fixed $x$, varying $y$)}
