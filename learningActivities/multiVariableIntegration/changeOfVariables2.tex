...
Let's build the bounds of integration:

The radius $r$ goes from the center to the edge of the cylinder: $\answer{0} \leq r \leq \answer{2}$

The height $z$ goes from the bottom to the top of the cylinder: $\answer{0} \leq z \leq \answer{5}$

The angle $\theta$ goes all the way around the cylinder: $\answer{0} \leq \theta \leq \answer{2\pi}$

The integral setup for total mass:
$$M = \int_{\answer{0}}^{\answer{2\pi}} \int_{\answer{0}}^{\answer{5}} \int_{\answer{0}}^{\answer{2}} \answer{(1000 + 50r^2 - 20z) \cdot r}\,dr\,dz\,d\theta$$

This turns out to be only a couple of power rule applications, so we'll leave the rest to you. But the point is, the mass integral was fairly straightforward in both set up and execution once we chose the right coordinate system!
...
The mass element is: $dM = \delta \cdot dV = \answer{(3 + 2z) \cdot r}\,dr\,d\theta\,dz$
...
$$M = \int_{\answer{0}}^{\answer{2\pi}} \int_{0}^{5} \int_{0}^{4z/5} \answer{(3+2z) \cdot r}\,dr\,dz\,d\theta$$
...
\textbf{Where does $\rho^2\sin\phi$ come from?}

The following video from Dr. Pierce's Physics \& Math goes through a pretty quick and intuitive description of how to build spherical coordinates (in addition to cylindrical and cartesian).

\begin{center}
\youtube{K3bzF4WcE14?si=wmDD2cCOV6rFXmyB}
\end{center}
...
\textbf{The three tiny edge lengths} of the spherical box with volume $dV$ are: 

\begin{enumerate}

\item A purely radial step $d\rho$, as the radius changes outward from $\rho$ to $\rho + d\rho$;
\item a ``meridional'' (vertical) arc of length $\rho\,d\phi$ (sweeping angle $d\phi$ along a circle of radius $\rho$); 
\item and an ``azimuthal'' (horizontal) arc of length $\rho\sin\phi\,d\theta$ (sweeping angle $d\theta$ along the horizontal circle at colatitude $\phi$.

\end{enumerate}

Alltogether this makes the linearization $dV = \rho^2\sin\phi\,d\rho\,d\phi\,d\theta$.

\begin{problem}
Now check the ``Show Sphere'' box to explore spherical coordinates visually.
...
Keep in mind that for the ``vertical'' rotation downward, we only need to rotate around half the circle. The full circular rotation happens in the ``horizontal'' rotation next. 

So the bounds of $\phi$ are from $0 \leq \phi \leq \answer{\pi}$, 

The full rotation in $\theta$ is $0 \leq \theta \leq \answer{2\pi}$.

The mass element is $dM = \delta(\rho)\,dV = k\cdot \rho^3 \cdot \sin(\phi)\,d\rho\,d\phi\,d\theta$.

This makes the integral for total mass:

$$M = \int_{\answer{0}}^{\answer{2\pi}}\int_{\answer{0}}^{\answer{\pi}}\int_{\answer{0}}^{\answer{R}} k\cdot \rho^3\cdot \sin(\phi)\,d\rho\,d\phi\,d\theta$$

\textbf{Inner integral (in $\rho$):}
$$\int_0^R k\rho^3\sin(\phi)\,d\rho = \sin(\phi)\left[\answer{\frac{k\rho^4}{4}}\right]_0^R = \sin(\phi)\answer{k R^4/4}$$

\textbf{Middle integral (in $\phi$):}
$$\int_0^{\pi} \frac{kR^4}{4}\sin\phi\,d\phi = \frac{kR^4}{4}[\answer{-\cos(\phi)}]_0^{\pi}= \answer{kR^4/2}$$

\textbf{Outer integral (in $\theta$):}
$$\int_0^{2\pi} \frac{kR^4}{2}\,d\theta = \answer{k\pi R^4}$$
