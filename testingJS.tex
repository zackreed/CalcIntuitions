...
// Unicode symbol shortcuts (use escape sequences to avoid embedding raw unicode in TeX)
const PI_SYMBOL = '\u03C0'; // pi (unicode escape for pi)
const DELTA = '\u0394'; // Delta (unicode escape for Delta)

// Emoji constants (surrogate pairs) - ASCII-safe in the .tex file
const RABBIT = '\uD83D\uDC30'; // rabbit face
const TURTLE = '\uD83D\uDC22'; // turtle

// Canvas-drawn icon fallbacks (guaranteed, no external assets)
// (Removed canvas icon functions; racer dots now render as Unicode emoji glyphs)
...
function getOvalPosition(progress) {
    // Map progress to angle around oval (0 to 2pi)
...
function drawTrack() {
    ...
    // Draw hare (unicode emoji glyph)
    const hareProgress = hareStrategy(time);
    const harePos = getOvalPosition(hareProgress);
    const [hx, hy] = toCanvas(harePos[0], harePos[1]);
    const hareW = 36, hareH = 24;
    trackCtx.save();
    trackCtx.font = Math.round(hareH) + 'px serif';
    trackCtx.textAlign = 'center';
    trackCtx.textBaseline = 'middle';
    trackCtx.fillText(RABBIT, hx, hy);
    trackCtx.restore();

    // Draw tortoise (unicode emoji glyph)
    ...
    const tortW = 34, tortH = 22;
    trackCtx.save();
    trackCtx.font = Math.round(tortH) + 'px serif';
    trackCtx.textAlign = 'center';
    trackCtx.textBaseline = 'middle';
    trackCtx.fillText(TURTLE, tx, ty);
    trackCtx.restore();
}
...
function drawGraph() {
    ...
    if (time < 1 - dt) {
        ...
        // Hare dt arrows (horizontal Deltat, then vertical Deltay)
        ...
        // Tortoise dt arrows (horizontal Deltat, then vertical Deltay)
